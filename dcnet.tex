...
\tadAlinearFunciones{ubicacionPaqCompus}{dcnet/d , paq/p, conj(compu) cc}
	\tadOperacion{paquetesRecibidos} {dcnet/d , compu/c} {conj(paq)} {$c\in$ computadoras(topologia($d$))}
	\tadOperacion{siguientePC} {dcnet/d , compu/o , compu/d} {compu} {$o,d \in$ computadoras(topologia($d$)) \yluego \\
																				(\existe{nat}{n}, $n \leq$ \#computadoras(topologia($d$)))\\
																				$\neg\emptyset?$
	...
	\tadOperacion{ubicacionPaqCompus} {dcnet/d , paq/p, conj(compu) cc} {compu} {(\existe{compu}{c}, $c \in$ computadoras(topologia($d$))) $p\in$ elementos(paquetesEnPC($d$, $c$)) $\wedge$ \\ $cc \subseteq$ computadoras(topologia($d$))}
...
\tadAxiomas [\paratodo {dcnet}{d}, \paratodo {compu}{c, c', actual, destino}, \paratodo{topo}{t}, \paratodo{paq}{p}]
  ...
  \tadAxioma{recorridoPaquete(ingresarPaquete($d$, $c'$, $p$)}{\IF p $\neq$ p' THEN recorridoPaquete(r, p) ELSE <> FI}
  \tadAxioma{recorridoPaquete(siguienteSegundo($d$), $p$)}{
    \IF proximo(paquetesEnPC(r, ubicacionActual($d$, $p$))) = $p$ \\
    THEN recorridoPaquete(d) $\circulito$\\
